...
- web_search: Search the web. action_input: {"unknown": "search terms", "hypothesis": "optional guess", "max_results": 5}. Only unknown is sent to the search API. max_results optional (default 5).
...
- manage_state: CRUD on user_tasks, agent_tasks, reminders, finances, interests, audit_log, idle_memory, memories. action_input: {"table": "reminders", "operation": "select|insert|update|delete", "payload": {}}. payload may include owner (user|agent), source, and for reminders kind (user_reminder|agent_wakeup). Idle/heartbeat cannot create user-owned rows; use kind=agent_wakeup for the agent's own later resume. memories: kind=profile|fact|goal|note, content required; optional subject, evidence_id (tool observation id). status is assigned by the write path; durable observed facts require evidence_id. USER.md is a projection of memories — do not write it directly. select: optional completed/limit. update/delete: payload.id required. insert: reminders/tasks need scheduled_at+content; finances need amount+category; interests need name.
